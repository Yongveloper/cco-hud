---
description: Configure cco-hud as your statusline
...
Configure the cco-hud status line plugin with automatic runtime and platform detection.
...
1. Get plugin path (check cache/marketplace first, then manual install):
   ```bash
   ls -td ~/.claude/plugins/cache/cco-hud/cco-hud/*/ 2>/dev/null | head -1 || ls -d ~/.claude/plugins/cco-hud/ 2>/dev/null
   ```
...
4. Generate command with dynamic path detection:
   ```
   bash -c '"{RUNTIME_PATH}" "$(ls -td ~/.claude/plugins/cache/cco-hud/cco-hud/*/ 2>/dev/null | head -1 || ls -d ~/.claude/plugins/cco-hud/ 2>/dev/null){SOURCE}"'
   ```
...
1. Get plugin path:
   ```powershell
   $cache = "$env:USERPROFILE\.claude\plugins\cache\cco-hud\cco-hud"
   $manual = "$env:USERPROFILE\.claude\plugins\cco-hud"
...
4. Generate PowerShell command:
   ```
   powershell -Command "& {$cache='$env:USERPROFILE\.claude\plugins\cache\cco-hud\cco-hud';$manual='$env:USERPROFILE\.claude\plugins\cco-hud';$p=if(Test-Path $cache){(Get-ChildItem $cache|Sort-Object LastWriteTime -Descending|Select-Object -First 1).FullName}else{$manual}; & '{RUNTIME_PATH}' (Join-Path $p '{SOURCE}')}"
   ```

## Step 2: Create Configuration File

Create `~/.claude/cco-hud.local.json`:
...
**Plugin not found**:
- Install via: `/plugin marketplace add yongveloper/cco-hud`
