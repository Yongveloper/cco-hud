...
import { formatTokens, formatTimeRemaining, formatDaysRemaining, shortenModelName } from '../utils/formatters.js';
...
function buildRateLimitsSection(ctx: RenderContext, t: Translations): string | null {
  ...
  if (isMaxPlan && (limits.seven_day || limits.seven_day_sonnet)) {
    ...
    if (limits.seven_day) {
      ...
      const color = getColorForPercent(pct);
      let text = `${t.labels['7d_all']} ${colorize(`${pct}%`, color)}`;
      if (limits.seven_day.resets_at) {
        text += ` (${formatDaysRemaining(limits.seven_day.resets_at, t)})`;
      }
      sevenDayParts.push(text);
    }
    ...
    if (limits.seven_day_scoped) {
      const pct = Math.round(limits.seven_day_scoped.utilization);
      const color = getColorForPercent(pct);
      sevenDayParts.push(`${limits.seven_day_scoped.model}: ${colorize(`${pct}%`, color)}`);
    }
